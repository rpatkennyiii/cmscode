...
void HcalUnpacker::unpack(const FEDRawData& raw, const HcalElectronicsMap& emap,
  ...
  for (int spigot=0; spigot<HcalDCCHeader::SPIGOT_COUNT; spigot++) {
    ...
    */
	...
	if (did.null()) {
          if ((fiber<2||fiberchan<6)&&(spigot==12||spigot==13)){
	    colls.zdcCont->push_back(ZDCDataFrame(HcalZDCDetId(did)));
	    HcalUnpacker_impl::unpack_compact<ZDCDataFrame>(ptr_header, ptr_end, colls.zdcCont->back(), nps, eid, startSample_, endSample_, expectedOrbitMessageTime_, htr);
	  } 
        }

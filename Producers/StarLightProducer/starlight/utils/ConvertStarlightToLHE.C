...
void ConvertStarlightToLHE(int num=0)
{
  string filename = "slight.out"; //input
  ifstream infile(filename.c_str());
  char outfilename[100];
  sprintf(outfilename,"slight%i.lhe",num);
  ...
  const unsigned int M = 1000000; // N_events
  const unsigned int K = 1;  // first event 
 ...
 //  getline(infile,temp_string); // The very first line is useless. commented out by R. P. Kenny III.
